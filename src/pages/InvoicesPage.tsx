      ...
      <div className="p-8 space-y-6">
              ...
              <div>
                ...
                <div className="border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  <div className="px-4 py-3 border-t border-gray-100 bg-gray-50 flex justify-end gap-8 text-sm">
                    <span className="text-gray-500">Subtotal: <strong className="text-gray-900">{formatCurrency(subtotal)}</strong></span>
                    <span className="text-gray-500">Tax: <strong className="text-gray-900">{formatCurrency(taxAmount)}</strong></span>
                    <span className="text-gray-500">Total: <strong className="text-gray-900">{formatCurrency(subtotal + taxAmount)}</strong></span>
                  </div>
                </div>
              </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 max-w-xs">
            ...
            <input className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500" placeholder="Search invoices..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="px-6 py-10 text-center text-sm text-gray-400">No invoices found</div>
          ) : (
            filtered.map(inv => (
              <div key={inv.id} className="grid grid-cols-12 gap-3 px-6 py-3.5 border-b border-gray-50 hover:bg-gray-50 items-center text-sm">
                <span className="col-span-2 font-mono text-gray-700">{inv.number}</span>
                <span className="col-span-3 text-gray-900 font-medium">{inv.contacts?.name}</span>
                <span className="col-span-2 text-gray-600">{formatDate(inv.issue_date)}</span>
                <span className="col-span-2 text-gray-600">{formatDate(inv.due_date)}</span>
                <span className="col-span-1 text-right font-medium text-gray-900">{formatCurrency(inv.total)}</span>
                <span className="col-span-1"><Badge {...statusBadge(inv.status)}>{inv.status}</Badge></span>
                {isAccountant && inv.status === 'draft' && (
                  <button className="col-span-1 text-xs text-primary-600 hover:text-primary-700 font-medium" onClick={() => updateStatus(inv.id, 'sent')}>Mark Sent</button>
                )}
                {isAccountant && inv.status === 'sent' && (
                  <button className="col-span-1 text-xs text-green-600 hover:text-green-700 font-medium" onClick={() => updateStatus(inv.id, 'paid')}>Mark Paid</button>
                )}
              </div>
            ))
          )}
